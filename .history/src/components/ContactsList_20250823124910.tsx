...
function ContactsList({ onSelectChat }: ContactsListProps) {
  ...
  const loadContacts = async () => {
    try {
      setError('');
      // For loading all contacts, we need a different approach
      // since the search endpoint requires at least 2 characters
      const { data } = await searchUsers('all'); // Use a special keyword or empty search
