  ...
  useEffect(() => {
    ...
    const loadChatData = async () => {
      try {
        ...
        // Also load chat info for group chats
        try {
          const { data: chatsData } = await getChats();
          const chat = chatsData.find((chat: Chat) => chat._id === chatId);
          if (chat) {
            setCurrentChat(chat);
          }
        } catch (chatError) {
          console.log('Could not load chat info:', chatError);
        }
      } catch (error) {
      ...
      >
        ...
        {showScrollButton && (
          <button
            ...
            className="fixed bottom-20 right-4 bg-blue-500 text-white p-3 rounded-full shadow-lg hover:bg-blue-600 transition-colors z-20"
      ...
      <form onSubmit={handleSendMessage} className="p-3 bg-white border-t">
        <div className="flex space-x-2">
          <input
            ...
            className="flex-1 p-3 border rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 text-lg"
          />
          <button
            type="submit"
            disabled={!newMessage.trim()}
            className="px-5 py-3 bg-blue-500 text-white rounded-full hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed min-w-[60px] flex items-center justify-center text-lg"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
