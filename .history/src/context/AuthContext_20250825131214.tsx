...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const { setTheme, setNotifications } = useTheme(); // Now this is safe
    ...
    const loadUser = async () => {
      try {
        ...
        if (token) {
          ...
          if (data.preferences) {
            const { theme, notifications } = data.preferences;
            if (theme) setTheme(theme);
            if (notifications !== undefined) setNotifications(notifications);
          }
        } else {
  ...
  const login = async (emailOrMobile: string, password: string) => {
    try {
      ...
      if (data.user.preferences) {
        const { theme, notifications } = data.user.preferences;
        setTheme(theme);
